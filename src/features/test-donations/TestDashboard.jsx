...
import useGetSomeData from "./useGetSomeData";
import { CircularProgress } from "@mui/material";

function TestDashboard() {
  const { data: myData, isLoading: isLoadingMyData } = useGetSomeData();
  return (
    <div className="mx-auto w-[800px] mt-8">
      <Grid2 container spacing={2} columns={16}>
        ...
        <Grid2 md={16}>
          {isLoadingMyData ? <CircularProgress /> : <p>some data is loaded.</p>}
        </Grid2>
        <Grid2 sm={6} md={8}>
          <TestPieChart />
        </Grid2>
